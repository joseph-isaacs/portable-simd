...
/// Portable SIMD: 8 popcounts, in-register inclusive scan (3 shift+add steps), carry broadcast.
pub fn rank_index_portable(bits: &[u64], out: &mut [u32]) {
    ...
    for (c, o) in chunks.iter().zip(oc) {
        ...
        let excl = (incl - cnt) + Simd::splat(carry);
...
pub fn rank_index_avx2(bits: &[u64], out: &mut [u32]) {
    ...
    unsafe {
        ...
        for (c, o) in chunks.iter().zip(oc) {
            ...
            let excl = _mm256_add_epi64(_mm256_sub_epi64(incl, cnt), _mm256_set1_epi64x(carry));
            ...
            carry += _mm256_extract_epi64::<3>(incl);
        }
    }
    let mut acc = carry as u32;
    for (&w, o) in rem.iter().zip(orem) {
...
pub fn rank_index_avx512(bits: &[u64], out: &mut [u32]) {
    ...
    unsafe {
        ...
        for (c, o) in chunks.iter().zip(oc) {
            ...
            let excl = _mm512_add_epi64(_mm512_sub_epi64(incl, cnt), _mm512_set1_epi64(carry));
            _mm256_storeu_si256(o.as_mut_ptr() as *mut __m256i, _mm512_cvtepi64_epi32(excl));
            let lanes: [i64; 8] = core::mem::transmute(incl);
            carry += lanes[7];
        }
    }
    let mut acc = carry as u32;
