...
|---|---|---|---|---|
| **`Mask::from_bitmask` scalarises on AArch64** (~150 instrs / 64 B, 38 / 16 B) | `trunc i64 -> i16`, `bitcast i16 -> <16 x i1>`, `zext/select` (ideal IR) | **LLVM, AArch64 backend — already fixed in LLVM 23** | LLVM 22.x has no counterpart of X86's `combineToExtendBoolVectorInReg`; LLVM 23.1.0 adds `combineToExtendBoolVectorInReg` to `AArch64ISelLowering.cpp` (`tbl` broadcast + `cmtst` + `ushr`). Verified: `llc` from HEAD and `rustc 1.100.0-nightly` (LLVM 23.1.1) emit 10 instrs for 16 lanes and 26 for 64 (was 38 / 137) | nothing to do upstream; the pinned `nightly-2026-04-28` (LLVM 22.1.2) is what shows the hole. Rust master's `src/llvm-project` is on 23.1 already |
| **`to_bitmask` via `addv` on AArch64** (`cmeq`, `bic`, `ext`, `zip1`, `addv h`, `str h` per 16 B) | `icmp ne <16 x i8>`, `bitcast <16 x i1> -> i16` (ideal IR) | **LLVM — fixed in the LLVM 23 era** | `vectorToScalarBitmask` in `AArch64ISelLowering.cpp` gained a `v16i8` `ADDP` x3 special case (the text is in the 22.1.x tags, but the April nightly's 22.1.2 build did not take it; `llc` from HEAD and the LLVM 23.1.1 nightly do: `cmeq`, `bic`, `addp` x3, `umov` per 16 lanes) | nothing to do for 16 lanes. Remaining LLVM improvement: 32/64-lane masks are still 4 independent 16-lane reductions + `shl`/`orr` (32 instrs / 64 B); folding the `addp` tree across the four vectors (as `bytes_to_bits_neon` does, 16 instrs) needs a >128-bit path in the same function |
...
Priority if the goal is fixing things: (1) `simd_compress` plumbing onto
`llvm.experimental.vector.compress` (Rust; LLVM side already done), (2) the bit compress/expand
intrinsics (Rust API + codegen; largest payoff, most design work), (3) `to_bitmask` `addp` tree
across >16 lanes on AArch64 (LLVM, `vectorToScalarBitmask`), (4) `extract_dyn` / `prefix_sum`
